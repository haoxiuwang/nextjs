...
export default function ({height,auto,setAuto,fz,setFz,index,setIndex,time,repeat,setRepeat,texts,back,fav,setFav,serie,_key}) {
  ...
  var _local = local.map((item)=>if(item.index==index)item)
  console.log(_local);
  return(
  <div onTouchStart={(e)=>{
    e.stopPropagation()
    ...
    <div>
      ...
      {_local.length>0&&(<div className={`relative p-3 py-10 m-3 w-[20rem] md:w-[25rem] text-center bg-sky-100 border-solid rounded border-sky-400 border-1`}>
        {
          _local.map((item,i)=>{
          return(
