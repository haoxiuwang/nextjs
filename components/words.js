...
export default function Words({height,texts,index,str,setFav,_key}) {
  ...
  return (
      <div ref={ref2} className="h-full m-3 mt-16">
        ...
        <div>
          <div className="relative border-b-2 border-sky-700 border-solid my-12 mx-0 grid grid-cols-[min-content_1fr_min-content] gap-1">
            <div className="flex place-content-center place-items-center" onClick={()=>{
                setNotes([])
              ...
              <span className="ml-3" onClick={()=>window.open(`https://www.ldoceonline.com/dictionary/${word.en}`,'_blank')}>朗文</span>
              <span className="ml-3" onClick={()=>window.open(`https://www.bing.com/dict/search?q=${word.en}&mkt=zh-cn`,'_blank')}>Bing</span>
            </div>
            <input className="appearance-none text-base text-sky-600 placeholder:text-slate-600 focus:outline-none sm:text-sm sm:leading-6" value={word.en} onChange={(e)=>setWord({en:e.target.value,zh:""})}/>
            <span onClick={()=>setWord({en:"",zh:""})} >{word.en.length>0&&<svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" className="bi bi-x-lg" viewBox="0 0 16 16">
