...
export default function Words({height,texts,index,str,setFav,_key}) {
  ...
  return (
      <div ref={ref2} className="h-full m-3 mt-16">
        ...
        <div>
          <div className="relative border-b-2 border-sky-700 border-solid my-12 mx-0 grid grid-cols-[min-content_1fr_min-content] place-items-center gap-2">
            <div className="flex place-content-center place-items-center space-x-2" onClick={()=>{
                setNotes([])
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-book text-sky-800" viewBox="0 0 16 16">
              ...
              <span className="ml-3" onClick={()=>window.open(`https://idioms.thefreedictionary.com/${word.en.trim().indexOf(" ")>-1?word.en.trim().split(" ").join("+"):word.en.trim()}`,'_blank')}>C</span>
              <span className="ml-3" onClick={()=>window.open(`https://www.bing.com/dict/search?q=${word.en.trim()}&mkt=zh-cn`,'_blank')}>B</span>
            </div>
            <input className="appearance-none min-w-0 text-base text-sky-600 placeholder:text-slate-600 focus:outline-none sm:text-sm sm:leading-6" value={word.en} onChange={(e)=>setWord({en:e.target.value,zh:""})}/>
            <div className="flex place-content-center plex-items-center">
              <span onClick={()=>setWord({en:"",zh:""})} >{word.en.length>0&&<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-x-lg" viewBox="0 0 16 16">
      ...
      {notes.length>0&&word.en.length>0&&(notes[0]==""?(
        <div className="text-rose-400">Sorry!</div>):(<div className="bg-sky-100  p-3">
          {
            notes.map((item,i)=>(
              <span key={i} onClick={()=>setWord({en:(word.en).trim(),zh:word.zh+" "+item})} className="inline-block mr-2 rounded ring-1 bg-white p-1 px-2" _key={i}>{item}</span>
            ))
          }
        </div>))
      }
        <div>
          {
            words.map((item,i)=>(
              <span key={i} onClick={()=>setWord({en:(word.en+" "+item.toLowerCase()).replace(/[\,\.\;\?\"]/g," ").trim(),zh:word.zh})} className="inline-block mr-2" _key={i}>{item}</span>
            ))
          }
        </div>
          ...
          <div className="mt-1 text-slate-700 select-none">
            ...
            <ul className="block text-left">
            {
              local.map((item,i)=>{
                ...
                return(
                <li key={i} onClick={()=>setWord(item)} onContextMenu={()=>{
                  var arr = [...local]
                  arr.splice(i,1)
                  setLocal(arr)
                }} _key={i} className="mb-2 block leading-none">
                  <span className="font-semibold">{item.en}: </span><span>{item.zh}</span>
                </li>
                )
              })
            }
